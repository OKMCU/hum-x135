...
/* Private variables ---------------------------------------------------------*/
static xdata int8_t tidLight = -1;
static xdata int8_t tidMist = -1;
static xdata int8_t tidLightMist = -1;
static xdata int8_t tidPeriodCheck = -1;
static xdata uint8_t preTouchState = 0;
/* Private function prototypes -----------------------------------------------*/
/* Private functions ---------------------------------------------------------*/
static void appButtonLightLongPress(void)
{
    tidLight = -1;
    appEventLightButtonLongPress();
}
...
static void appButtonLightMistLongPress(void)
{
    tidLightMist = -1;
...
static void appButtonPeriodCheck(void)
{
    ...
    if(curTouchState != preTouchState)
    {
        ...
        {
            case APP_BUTTON_STATE_LIGHT:
                if(preTouchState == 0)
                {
                    appEventLightButtonTouchEnter();
                    if(tidLight < 0)
                        tidLight = appTaskSchedCreate(BUTTON_LONG_PRESS_TIME, appButtonLightLongPress);
                }

                if(tidLightMist >= 0)
                {
                    appTaskSchedDelete(tidLightMist);
                    tidLightMist = -1;
                }
            break;
            ...
            case (APP_BUTTON_STATE_LIGHT + APP_BUTTON_STATE_MIST):
                if(tidLight >= 0)
                {
                    appTaskSchedDelete(tidLight);
                    tidLight = -1;
                }
                if(tidMist >= 0)
                {
                ...
                if(tidLightMist < 0)
                {
                    appEventLightMistButtonTouchEnter();
                    tidLightMist = appTaskSchedCreate(BUTTON_VLONG_PRESS_TIME, appButtonLightMistLongPress);
                }
            break;
...
uint8_t appButtonGetCurrentState(void)
{
    ...
    if(halButtonGetState(HAL_BUTTON_Light) == HAL_BUTTON_STATE_Pressed)
    {
        state |= (1<<HAL_BUTTON_Light);
    }
